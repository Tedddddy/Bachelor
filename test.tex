...
\begin{example}
    Let $\phi = \Box p$ and $M = (W,R,V)$ with $W = \{w_1,w_2,w_3,w_4,w_5\}$, $R = \{(w_1,w_2), (w_2,w_3), (w_2,w_4), (w_2,w_5)\}$ and $V(p) = \{w_3,w_4,w_5\}$. (See figure 1.)
        \begin{figure}[h]
        \centering
        ...
        \end{figure} 
\end{example}
However, the term 'modal logic' is often used more broadly for a family of related systems. This include for example
...
\subsection{Topological semantics} An idea to generalize Kripke semantics was topology. 
The study of topological semantics in modal logic was initiated by McKinsey and Tarski in 1944 [3]. To assign meaning to modal formulas, we use topological space, which is a pair consisting of a domain 
...
\begin{example}
    Let $ W = \{1,2,3\}, \tau = \{\emptyset, \{1\}, \{2\}, \{1,2\}, W\}$ (where $\tau$ is the collection of open sets) and $V(p) = W$. Furthermore, let $\phi = \Box p$.
    \begin{figure}[h]
        \centering
        \begin{tikzpicture}[scale=1]
...
It is well known that the logic for any topological space is $S4$, where $S4$ is the modal logic with the transitivity ($\Box p \rightarrow \Box\Box p$) and reflexivity axiom ($\Box p \rightarrow p$). 
...
Topological semantics provides a rich, and intuitive semantics for $S4$ and related logics. It has a variety of applications e.g. spatial reasioning or program verification. Despite its rich expressiveness, it can not fully capture Kripke semantics. 
...
\begin{example}
    ...
    \centering
        ...
        \draw [mydarkgreen](0,0) circle (0.5cm);
        \draw [blue](-1.7,-1.6) circle (0.5cm);
        \draw [mydarkgreen](-0.1,-1.3) circle (2.3cm);
        \draw [blue](-0.1,-1.3) circle (2.4cm);
        \draw [red](1,-1.5) circle (0.5cm);
        \draw [mydarkgreen](1,-1.5) circle (0.6cm);

        \draw [blue](-0.7, -0.7) ellipse [x radius=0.7cm, y radius=2cm, rotate=-45];
        \draw [red](0.6, -0.8) ellipse [x radius=0.7cm, y radius=2cm, rotate=30];
        \end{tikzpicture}
...
is the minimal normal modal logic with three modal operators.

We focus on the logic of the product neighbourhood frames $F_1 \times^+_n F_2$ , where both $F_1$ and $F_2$ validates T.
The product $\times^+_n$ combines three types of functions : horizontal,vertical and standard. We will introduce the horizontal and vertical modalities which correspond to reasoning over the first and second coordinate frames, respectively,
while the standard modality reflects interactions in the combined frame. More specific definitions can be found in Chapter 3.2. \newline
...
corresponds to the logic of such product frames  introduced above — and vice versa.
...
The frame can be interpreted as a topological space which plays a crucial role in constructing a topo-bisimiulation between $T_{6,2,2}$ and a specific product topological space $\mathbb{Q} \times^+_n \mathbb{Q}$.   

In the second work [2], it was shown that for any $L_1, L_2$ $\in \{D,D4,T,S4\}$ the fusion $L_1 \otimes L_2$
agrees with the logic of product of neighbourhood frames but without using the standard neighborhood product.
...
6.2. By considering the product of these neighbourhood frames, the authors then construct a p-morphism to establish the desired result. 
...
Then, we consider its neighbourhood versions and take their product, constructing a p-morphism to establish the desired result. \newline
...
In \textbf{Chapter 4}, we begin the first part of our contribution. We formally introduce the infinitely branching and infinite depth tree. Then, using Filtration Theorem, the Sahlqvist Theorem 
and an unravelling technique to show that our logic is sound and complete with respect to this tree. \newline

\textbf{Chapter 5} is dedicated to the author's work presented in [1]. We provide a more detailed 
...
% In this work, we start with a brief recap of necessary notation and theorems in Kripke semantics and then introduce toplogical spaces and neighborhood frames. 
...
\begin{theorem}
    ...
    \begin{proof}
        ...
        Case $\phi = p$: Both direction follows immediately from filtration definition. \newline
        
        Case $\phi = \neg \psi$: $M,w \vDash \phi \Leftrightarrow M,w \nvDash \psi \overset{\text{IH}}{\Leftrightarrow} M,[w] \nvDash \psi \Leftrightarrow M,[w] \vDash \phi$. \newline
        
        Case $\phi = \phi_1 \lor \phi_2$: Let $M,w \Vdash \phi$. That means $M,w \Vdash \phi_1$ or $M,w \Vdash \phi_2$. Applying
        ...
        Case $\phi = \Box_i\psi$ : Left to right. Suppose $M,w \Vdash \Box_i \psi$. 
        ...
        Right to left. Suppose $M^f,[w] \Vdash \Box_i \psi$. Pick $v \in W$ s.t $wR_iv$. By condition 2, w.r.t to the modal operator, we have
...
\begin{lemma}
    ...
    It suffices to show $R^s_i$ fulfills the condition 2 and 3 w.r.t to the corresponding modal operator $\Box_i$. But $R^s_i$ already satisfies condition 2.
...
\begin{proposition}
    ...
    \begin{proof}
    Let $g : W_\Sigma \rightarrow P(\Sigma)$ defined by $g([w]) = \{ \phi \in \Sigma \mid M,w \Vdash \phi\}$.
    ...
    By assumption we have $u \equiv v$. Pick any $u' \in [u]$. Then we have $u' \equiv u \equiv v$. Hence, $u' \in [v]$. Therefore, $M^f$ contains at most $2^n$ nodes.
...
It is also well known, that the modal logic of any topological space is $S4$.
\newline
Neighborhood frames generalizes the idea of topology by loosen the conditions that define open sets in topological space.
...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \caption{vertically}
  \end{subfigure}
  \hspace{1cm} % horizontal space between the figures
  \begin{subfigure}[b]{0.3\textwidth}
    \centering
    \begin{tikzpicture}
      \draw (0,0) rectangle (3,2);
  ...
  \end{subfigure}
  \caption{Each rectangle represents $N$ and the redlines are subsets of $N$} 
\end{figure}
...
\begin{definition}
    ...
    where $\mathcal{X},\mathcal{Y}$ are topological spaces.
\end{definition}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[scale=1]
    ...
    \draw [blue] (4.4,1.1) -- (5.8,1.1) -- (5.8,2.3) -- (4.4,2.3) -- cycle;



  \end{tikzpicture}
  \caption{This is an illustration for $\tau_1'$ with a fix point $(x,y)$. The red lines represents the combinations of any neighbourhood of x with y.
...
The idea is to pick a class of frame $\mathcal{C}$ s.t $Log(\mathcal{C}) = Log(T_{\omega, \omega, \omega[rn]})$ and then show the class has FMP.
...
\begin{definition}
    ...
    Formally the tree can be defined as : $T_{\omega [rn]} = (W, R)$ where $W = \mathbb{N}^*$,  $$sRt \text{ iff } \exists u \in \mathbb{N} \cup \{\epsilon\} : s \cdot u = t$$ ("$\cdot$" is the concatenation operator) 
    \clearpage
...
$S4 \otimes S4 \otimes S4 + \Box p \rightarrow \Box_1 p \land \Box_2 p$ [1]. First we will show a version for one modality
...
\begin{figure}[h]
    \centering
        \begin{tikzpicture}
            ...
            \draw[fill=black] (-5.5,0) circle (1pt) node[below] {0};;
            \draw(-7.1,0.1) -- (-7.1,-0.1) node[below] {-$\frac{1}{3}$};
            \draw(-8.8,0.1) -- (-8.8,-0.1) node[below] {-$\frac{2}{3}$};
            \draw(-10.5,0.1) -- (-10.5,-0.1) node[below] {-1};
            \draw(-12.3,0.1) -- (-12.3,-0.1) node[below] {-$\frac{4}{3}$};

            \draw(-3.7,0.1) -- (-3.7,-0.1) node[below] {$\frac{1}{3}$};
            \draw(-1.8,0.1) -- (-1.8,-0.1) node[below] {$\frac{2}{3}$};
            \draw(0.1,0.1) -- (0.1,-0.1) node[below] {1};
            \draw(2.2,0.1) -- (2.2,-0.1) node[below] {$\frac{4}{3}$};

            \draw [red] (-10.5,0.15) -- (-10.5,-0.15);
            \draw [red] (0.1,0.15) -- (0.1,-0.15);

            \draw[blue] (2.2,0.2) -- (2.2,-0.2);
            \draw[blue] (2.2,0.2) -- (2.1,0.2);
            \draw[blue] (2.2,-0.2) -- (2.1,-0.2);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            \draw [blue] (-1.8,0.2) -- (-1.8,-0.2);
            \draw [blue](-1.8,0.2) -- (-1.7,0.2);
            \draw [blue] (-1.8,-0.2) -- (-1.7,-0.2);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            \draw [blue] (-7.1,0.2) -- (-7.1,-0.2);
            \draw [blue] (-3.7,0.2) -- (-3.7,-0.2);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            \draw [blue] (-12.3,0.2) -- (-12.3,-0.2);
            \draw [blue] (-12.3,0.2) -- (-12.2,0.2);
            \draw [blue] (-12.3,-0.2) -- (-12.2,-0.2);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            \draw [blue] (-8.8,0.2) -- (-8.8,-0.2);
            \draw [blue] (-8.8,0.2) -- (-8.9,0.2);
            \draw [blue] (-8.8,-0.2) -- (-8.9,-0.2);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            \draw[very thick, mydarkgreen] (-6.0,0.2) -- ( -6, -0.2); 
            \draw[very thick, mydarkgreen] (-5.0,0.2) -- ( -5, -0.2); 

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            \draw[very thick, mydarkgreen] (-6.6,0.2) -- ( -6.6, -0.2);
            \draw[very thick, mydarkgreen] (-6.6,0.2) -- ( -6.7, 0.2); 
            \draw[very thick, mydarkgreen] (-6.6,-0.2) -- ( -6.7, -0.2);
            

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            \draw[very thick, mydarkgreen] (-7.6,0.2) -- ( -7.6, -0.2);
            \draw[very thick, mydarkgreen] (-7.6,0.2) -- ( -7.5, 0.2); 
            \draw[very thick, mydarkgreen] (-7.6,-0.2) -- ( -7.5, -0.2);

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            \draw[very thick, mydarkgreen] (-8.2,0.2) -- (-8.2,-0.2);
            \draw[very thick, mydarkgreen] (-9.4,0.2) -- (-9.4,-0.2);

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            \draw[very thick, mydarkgreen] (-11.1,0.2) -- (-11.1,-0.2);
            \draw[very thick, mydarkgreen] (-11.1,0.2) -- (-11,0.2);
            \draw[very thick, mydarkgreen] (-11.1,-0.2) -- (-11,-0.2);

            \draw[very thick, mydarkgreen] (-9.9,0.2) -- (-9.9,-0.2);
            \draw[very thick, mydarkgreen] (-9.9,0.2) -- (-10,0.2);
            \draw[very thick, mydarkgreen] (-9.9,-0.2) -- (-10,-0.2);

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            \draw[orange] (-5.65,0.2) -- (-5.65, -0.2);
            \draw[orange] (-5.65,0.2) -- (-5.55, 0.2);
            \draw[orange] (-5.65,-0.2) -- (-5.55, -0.2);

            \draw[orange] (-5.35,0.2) -- (-5.35, -0.2);
            \draw[orange] (-5.35,0.2) -- (-5.45, 0.2);
            \draw[orange] (-5.35,-0.2) -- (-5.45, -0.2);

                \draw[orange] (-6.2,0.2) -- ( -6.2, -0.2);
                \draw[orange] (-5.8,0.2) -- ( -5.8, -0.2);  
                \draw[orange] (-4.8,0.2) -- ( -4.8, -0.2);
                \draw[orange] (-5.2,0.2) -- ( -5.2, -0.2);

        \end{tikzpicture}
        \caption{{\footnotesize This is an excerpt of $X$. 
        Some points are visualized as brackets to distinguish them from nearby points at the same level.}}
    \end{figure}


In Figure 3, each colour represents a different level. The points in red are from $X_1$, blue points from $X_2$, green points from a part of $X_3$ and orange from a part of $X_4$. 
We can observe that, the distance between a point and its source is getting exponentially smaller.
...
\begin{lemma}
    ...
    \begin{proof}
        ...
        If $k > max\{st(\alpha), st(\beta)\}$ then we are done. Else, by Lemma 5.2.3, we have for any $n \geq k : U_n(\alpha) \subseteq U_k(\alpha)$. So we can lift the $k$ til we reach $m$. Then we use Lemma 5.2.9 to get the following :
        ...
        We need to prove there exists $U \in \tau_1'(\alpha,\beta)$, such that $g(U) \subseteq V$. As $U$ we take $U_m(\alpha) \times \{\beta\}$ for some $m > max\{st(\alpha), st(\beta)\}$. Hence, by Lemma 5.2.9 we get :
