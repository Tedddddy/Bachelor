...
\begin{itemize}
    \item If \( \tau = \{\emptyset, X\} \), then \( (X, \tau) \) is called the \textbf{trivial topological space}.
    
    \item If \( \tau = \mathcal{P}(X) \), then \( (X, \tau) \) is called the \textbf{discrete topological space}.
    
    \item If \( \tau = \{\emptyset, \{1\}, \{2\}, \{1,2\}, X\} \), then \( (X, \tau) \) is \textbf{topological space}.
\end{itemize}
\end{example}
...
\begin{definition}
    Let \( A = (X, \chi) \) and \( B = (Y, \upsilon) \) be topological spaces, and let \( N \subseteq X \times Y \).\newline \newline
We define
