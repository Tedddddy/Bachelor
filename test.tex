...
\begin{example}
    ...
    \centering
        ...
        \draw [ultra thick, mydarkgreen](0,0) circle (0.5cm);
        \draw [ultra thick, blue](-1.7,-1.6) circle (0.5cm);
        \draw [ultra thick,mydarkgreen](-0.1,-1.3) circle (2.3cm);
        \draw [ultra thick, blue](-0.1,-1.3) circle (2.4cm);
        \draw [ultra thick, red](1,-1.5) circle (0.5cm);
        \draw [ultra thick, mydarkgreen](1,-1.5) circle (0.6cm);

        \draw [ultra thick, blue](-0.7, -0.7) ellipse [x radius=0.7cm, y radius=2cm, rotate=-45];
        \draw [ultra thick, red](0.6, -0.8) ellipse [x radius=0.7cm, y radius=2cm, rotate=30];
        \end{tikzpicture}
...
\begin{theorem}
    ...
    \newline
    1. $\phi$ canonical \newline
    2. the class of Kripke frames validating $\phi$ can be characterized by a first order formula \newline
    3. $\phi$ is complete with respect to its class of validating frames
...
\end{figure}
    ...
    We denote $\tau_1$ is the set of all H-open subsets of $X \times Y$ and $\tau_2$ is the set of all V-open subsets of $X\times Y$. We say $\tau$ is the set of all standard product of subsets $X \times Y$
    with basis $\{X \times Y \mid X \in \chi \text{ and } Y \in \upsilon\}$.
...
\begin{definition}
    ...
    3. $\mbox{If }U, V \in \tau(x)$, then $U \cap V \in \tau(x)$ (closed under finite intersection)
\end{definition}
...
\begin{proposition}
    ...
    Pick [w],[v] s.t $[w]R^s_1[v]$. By defintiion of smallest filtration there are points $w' \in [w]$ and $v' \in [v]$ s.t $w'R_1v'$. Furthermore, it holds $R_1 \subseteq R$, so $w'Rv'$.
...
In Figure 7, each colour represents a different level. The points in red are from $X_1$, blue points from $X_2$, green points from a part of $X_3$ and orange from a part of $X_4$. 
...
\begin{proposition}
    ...
    \begin{proof}
        ...
        By definition of $f$, $f(y')$ is either left or right successor of $f(y)$ and by $f(y)= t' R t$, we get $f(y') = t$. It follows $f$ is open. \newline
...
\begin{theorem}[\textnormal{[1]}]
    $TPL$ is complete w.r.t. $\mathbb{Q} \times^+_t \mathbb{Q}$.
...
on $X \times^+_t X$ by $\xi(p) = h^{-1}(v(p))$. Since $h$ is continuous and open w.r.t. all three topologies and $h(0,0) = r$,
we have that $(0,0)$ and $r$ are 3-topo-bisimilar. Therefore, $(X \times^+_t X, \xi),(0,0) \nvDash \phi$. Similar as in Theorem 5.0.3, we argue
that $X \times^+_t X$ is homeomorphic to $\mathbb{Q} \times^+_t \mathbb{Q}$ w.r.t. all three topologies, it follows that $\phi$ is refutable on $\mathbb{Q} \times^+_t \mathbb{Q}$.
...
\begin{lemma}
    Let $F = (A^*, R) = F_{\xi \eta}[A]$. Based on that, let $N_\omega(F) = (X,\tau)$, $N(F) = (A^*, \sigma)$ and $f_F : N_\omega(F) \rightarrow N(F)$.
    Then for any $m \in \mathbb{N}$ and $x\in X$ with $x = a_1a_2...$ \newline we have $$f_F(U_m(x)) = R(f_F(x))$$
    \begin{proof}
            $\subseteq$ : Let $f_F(\alpha) \in f_F(U_m(x))$ with $\alpha \in U_m(x)$. Because $\alpha \in U_m(x)$, we have by definition of $U_m(x)$: 
            $x\mid_m = \alpha \mid_m$ and $f_F(x)R f_F(\alpha)$. Hence $f_F(\alpha) \in R(f_F(x))$.  \newline \newline
...
\begin{lemma}
    ...
    \begin{proof}
        ...
        For the next condition, assume that $x \in X \mbox{ and } U \in \tau(x)$. We need to prove that $f(U) \in \sigma(f(x))$. That is equivalent to $R(f(x)) \subseteq f(U)$. Because $U \in \tau(x)$, there is a $m$ such that
...
\begin{proposition}
    ...
    \begin{proof}
        ...
        It is easy to check that $X \vDash D$ iff for each $x \in X : \emptyset \notin \tau(x)$. In other words, if $\Box p \rightarrow \lozenge p$ is valid on $X$, then $\emptyset$ can not be a neighborhood for any point. For $N_\omega(F_{in})$ and $N_\omega(F_{it})$ this holds. \newline
        It is easy to check that $X \vDash T$ iff we have $x\in U \in \tau(x)$ for any $x$ and $U$. If $\Box p \rightarrow p$ is valid on $X$, then any neighborhood of a point contains the point itself. For $N_\omega(F_{rn})$ and $N_\omega(F_{rt})$ this holds. \newline
        Now we check $X \vDash 4$ iff for each $U \in \tau(x) : \{y \mid U \in \tau(y)\} \in \tau(x)$. \newline
        $\Leftarrow : $ Let $x \in X$ and assume $X,x \vDash \Box p$. That means there exists $U \in \tau(x)$ s.t $U\subseteq V(p)$.
        By assumption we have $S = \{y \mid U \in \tau(y)\} \in \tau(x)$. But then $X,x \vDash \Box\Box p$ because we can just pick the set S. \newline
        $\Rightarrow :$ This direction is by contradiction. Let's fix a valuation function $V$ s.t. $V(p) = U$. Furthermore, assume there exists a $U$ s.t $\{y \mid U \in \tau(y)\} \notin \tau(x)$ and let $X,x \vDash \Box p$.
        If $X,x \vDash \Box\Box p$ then it must be the case that $S =\{y \in X \mid X,y \vDash \Box p\} \in \tau(x)$. 
        \newline By $X,y \vDash \Box p$, it follows $U \in \tau(y)$. That means $S = \{y \in X \mid U \in \tau(y)\}$. But by assumption $S \notin \tau(x)$. 
...
\begin{lemma}
    ...
    where $g$ has arity of 2 and $g(U_m(\alpha) \times \{\beta\}) = \{g(\gamma, \beta) \mid \gamma \in U_m(\alpha)\}$
    \begin{proof}
        ...
        By choosing $m > max \{st(\alpha), st(\beta)\}$  and $\zeta \mid_m = \alpha \mid_m$, we have $g(\zeta \mid_m, \beta \mid_m) = g(\alpha,\beta)$. We also know $f_F(\alpha)R_1 f_F(\zeta)$, that means there exists a $\vec{d} \in A$ s.t. $f_F(\alpha) \cdot \vec{d} = f_F(\zeta)$. This $\vec{d}$ must appear at a point after $\zeta_m$.
...
\begin{lemma}
    ...
    \begin{proof}
        ...
        Assume $(\alpha,\beta) \in X_1 \times X_2$ and $U \in \tau_1'(\alpha, \beta)$. We have to show $g(U) \in \sigma_1(g(\alpha,\beta))$. That is equivalent to $R_1'(g(\alpha,\beta)) \subseteq g(U)$.
        The idea is to pick any $m > max\{st(\alpha), st(\beta)\}$ s.t. $U_m(\alpha) \times \{\beta\} \subseteq U$. We can do it, because $U \in \tau_1'(\alpha,\beta)$, so there exists $U_k(\alpha) \in \tau(\alpha)$ s.t. $U_k(\alpha) \times \{\beta\} =  \{(\gamma,\beta) \mid \gamma \in U_k(\alpha)\}= U$.
        If $k > max\{st(\alpha), st(\beta)\}$ then can just pick it. Else, by Lemma 6.2.3, we have for any $n \geq k : U_n(\alpha) \subseteq U_k(\alpha)$. So we can lift the $k$ until $k$ is greater than $max\{st(\alpha), st(\beta)\}$. Then we use Lemma 6.2.9 to get the following :
...
\subsection{Adapting the key ideas}
    ...
    \begin{lemma}
        ...
        We recall, $g(U_m(\alpha) \times U_m(\beta)) = \{(\zeta,\upsilon) \mid \zeta \in U_m(\alpha), \upsilon \in U_m(\beta)\}$. 
         \newline
        ...
        By choosing $m> max\{st(\alpha),st(\beta)\}$ we get $\alpha \mid_m = \zeta \mid_m$ and $\beta \mid_m = \upsilon \mid_m$, and hence $g(\alpha,\beta) = g(\zeta \mid_m, \upsilon \mid_m)$. Furthermore, we have $f_F(\alpha)R_1f_F(\zeta)$ and $f_F(\beta)R_2f_F(\upsilon)$, that means $\exists \vec{d} \in \mathbb{N}_1 \cup \{\epsilon\}$ and $\exists \vec{e} \in \mathbb{N}_2 \cup \{\epsilon\}$ s.t.
