...
these operators are interpreted over a set of possible worlds connected by an accessibility relation. 
\begin{example}
    Let $\phi = \Box p$ and $M = (W,R,V)$ with $W = \{w_1,w_2,w_3,w_4,w_5\}$, $R = \{(w_1,w_2), (w_2,w_3), (w_2,w_4), (w_2,w_5)\}$ and $V(p) = \{w_3,w_4,w_5\}$.
\end{example}
...
The study of topological semantics in modal logic was initiated by McKinsey and Tarski in 1944 [3]. 
It is well known, that the logic for any topological space is $S4$ where $S4$ is the logic with the transitivity ($\Box \Box p \rightarrow \Box p$) and reflexivity axiom ($\Box p \rightarrow p$). This is true, due to the topological semantics. We will discuss it later. Despite its rich expressiveness,
it can not fully capture Kripke semantics. 
